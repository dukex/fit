...
import { Controller as SettingsTabController } from "./src/SettingsTab";
...
export default class GithubSyncPlugin extends Plugin {
	...
	// 				"Settings not configured, please complete the following action items:\n" +
	// 				actionItems.join("\n");
	// 			const settingsNotice = new FitNotice(
	// 				this.fit,
	// 				["static"],
	// 				initialMessage,
	// 			);
	...
	// 	saveLocalStoreCallback = async (
	// 		localStore: Partial<LocalStores>,
	// 	): Promise<void> => {
	...
	// 	catchErrorAndNotify = async <P extends unknown[], R>(
	// 		func: (notice: FitNotice, ...args: P) => Promise<R>,
	// 		notice: FitNotice,
	// 		...args: P
	// 	): Promise<R | true> => {
	// 		try {
	...
	// 					case "getRef":
	// 						console.error(
	// 							"Caught error from getRef: ",
	// 							error.message,
	// 						);
	// 						if (error.status === 404) {
	// 							notice.setMessage(
	// 								"Failed to get ref, make sure your repo name and branch name are set correctly.",
	// 								true,
	// 							);
	// 							return true;
	// 						}
	// 						notice.setMessage(
	// 							"Unknown error in getting ref, refers to console for details.",
	// 							true,
	// 						);
	// 						return true;
	...
	// 			notice.setMessage(
	// 				"Unable to sync, if you are not connected to the internet, turn off auto sync.",
	// 				true,
	// 			);
	// 			return true;
	// 		}
	...
	// 		this.fitSyncRibbonIconEl = this.addRibbonIcon(
	// 			"github",
	// 			"Fit Sync",
	...
	// 					"Initiating sync",
	// 				);
	// 				const errorCaught = await this.catchErrorAndNotify(
	// 					this.sync,
	// 					syncNotice,
	// 				);
	...
	// 	async autoUpdate() {
	// 		if (
	// 			!(this.settings.autoSync === "off") &&
	// 			!this.syncing &&
	// 			!this.autoSyncing &&
	// 			this.checkSettingsConfigured()
	// 		) {
	// 			if (
	...
	// 				if (updated) {
	// 					const initialMessage =
	// 						"Remote update detected, please pull the latest changes.";
	// 					const intervalNotice = new FitNotice(
	// 						this.fit,
	// 						["static"],
	// 						initialMessage,
	// 					);
	...
	async onload() {
		...
		this.addSettingTab(new SettingsTabController(this));
	...
	async loadData() {
		const savedData = await super.loadData();
		const data = Object.assign({ settings: {} }, savedData);
	...
	// 			await this.loadData(),
	// 		);
	// 		const localStoreObj: LocalStores = Object.keys(
	// 			DEFAULT_LOCAL_STORE,
	// 		).reduce((obj, key: keyof LocalStores) => {
	...
	async saveSettings(settings: Partial<Settings>) {
		await this.saveData(
			Object.assign({}, await this.loadData(), { settings }),
		);

		console.log("settings saved", await this.loadData());
	}
	// 	async saveSettings() {
