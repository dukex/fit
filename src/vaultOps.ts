...
export class VaultOperations implements IVaultOperations {
	...
	async deleteFromLocal(path: string): Promise<FileOpRecord> {
		...
		throw new Error(
			`Attempting to delete ${path} from local but not successful, file is of type ${typeof file}.`,
		);
	}
	...
	async writeToLocal(path: string, content: string): Promise<FileOpRecord> {
		...
		throw new Error(
			`${path} writeToLocal operation unsuccessful, vault abstractFile on ${path} is of type ${typeof file}`,
		);
	}
